...
def bbox_intersections(boxes: np.ndarray, query_boxes: np.ndarray) -> np.ndarray:
    ...
    Returns
    -------
    overlaps: (N, K) ndarray of intersection between boxes and query_boxes
    """
    N = boxes.shape[0]
    K = query_boxes.shape[0]
...
# Compute bounding box voting
def box_vote(dets_NMS: np.ndarray, dets_all: np.ndarray) -> np.ndarray:
    """
    Parameters
    ----------
    dets_NMS: (N, 5) ndarray of float (NMS detections)
    dets_all: (M, 5) ndarray of float (all detections)
    Returns
    -------
    dets_voted: (N, 5) ndarray of voted detections
    """
    dets_voted = np.zeros((dets_NMS.shape[0], dets_NMS.shape[1]), dtype=np.float32)
    N = dets_NMS.shape[0]
    M = dets_all.shape[0]
    ...
    for i in range(N):
        ...
        for m in range(M):
            ...
            if ov < 0.5:  # Using a threshold of 0.5
                continue
