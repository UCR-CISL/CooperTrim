...
class Communication(nn.Module):
    ...
    def forward(self, batch_confidence_maps, B):
        ...
        for b in range(B):
            ...
            # if self.threshold:
            #     # Compute R = 1 - communication_maps
            #     R = 1.0 - communication_maps
            #     if L == 1:  # Only ego vehicle, no other agents
            #         filtered_communication_maps = communication_maps
            #     else:
            #         # Ego map remains unchanged, non-ego maps are scaled by ego map * R[1:]
            #         ego_map = communication_maps[0:1]  # Shape (1, 1, H, W)
            #         non_ego_R = R[1:]  # Shape (L-1, 1, H, W)
            #         non_ego_filtered = ego_map * non_ego_R  # Broadcasting ego_map across non-ego
            #         # Combine ego and filtered non-ego maps
            #         filtered_communication_maps = torch.cat([ego_map, non_ego_filtered], dim=0)  # Shape (L, 1, H, W)

            #     # Apply threshold to the filtered communication maps
            #     ones_mask = torch.ones_like(filtered_communication_maps).to(filtered_communication_maps.device)
            #     zeros_mask = torch.zeros_like(filtered_communication_maps).to(filtered_communication_maps.device)
            #     communication_mask = torch.where(filtered_communication_maps < self.threshold, ones_mask, zeros_mask)
            # else:
            #     communication_mask = torch.ones_like(communication_maps).to(communication_maps.device)
    ...
    def forward(self, x, psm_single, record_len, pairwise_t_matrix, backbone=None):
        ...
        if self.multi_scale:
            ...
            if self.fully:
                ...
                communication_masks, communication_rates = self.naive_communication(batch_confidence_maps, B)
                x = x * communication_masks
                # print(f"zeros and ones:{torch.sum(communication_masks == 0).item()}, {torch.sum(communication_masks == 1).item()}")
